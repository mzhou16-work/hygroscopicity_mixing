...
!     .. Parameters ..
      ...
      PARAMETER ( MXANG = 1025, LL = 7000, ZERO = 0.0D0, ONE = 1.0D0,& 
                 TWO = 2.0D0 )
!     ..
!     .. Scalar Arguments ..
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^zjl^^^^^^^^^^^^^^^^^^^^^^^^^^^^
!  change cos angle value to positive
      if(MU(1).LT. -TOLER .OR. MU(1).GT.1.0+TOLER )then
...
!                      ** Initialization procedure for stratified sphere
      ...
      BC  = 1.5D0*BCOE
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
!     write(*,*)ARE,BRE,ARE**2,AIM**2,BRE**2,BIM**2
!     write(*,*)( ARE + BRE ),( ARE**2 + AIM**2 + BRE**2 + BIM**2 )
!     write(*,*)'**********************'
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      DO 60 J = 1, NUMANG
